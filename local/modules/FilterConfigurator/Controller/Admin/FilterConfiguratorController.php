...
class FilterConfiguratorController extends BaseAdminController
{
    ...
    public function updateAction()
    {
        ...
        try {
            ...
            $this->setupFormErrorContext(
             $this->getTranslator()->trans("Error on edit configurator : %message", ["message" => $e->getMessage()], \FilterConfigurator\FilterConfigurator::DOMAIN_NAME), $e->getMessage(), $form
            );
    ...
    public function saveAction()
    {
        ...
        try {
            ...
            $this->setupFormErrorContext(
             $this->getTranslator()->trans("Error on new configurator : %message", ["message" => $e->getMessage()], \FilterConfigurator\FilterConfigurator::DOMAIN_NAME), $e->getMessage(), $form
            );
    ...
    public function saveImage($id)
    {
        ...
        try {
            ...
            $this->setupFormErrorContext(
             $this->getTranslator()->trans("Error on edit image for configurator : %message", ["message" => $e->getMessage()], \FilterConfigurator\FilterConfigurator::DOMAIN_NAME), $e->getMessage(), $form
            );
    ...
    public function saveImageAction($id)
    {
        ...
        return $this->saveFileAjaxAction(
          $id, 'configurator', $config['objectType'], $config['validMimeTypes'], $config['extBlackList']
        );
    }

    public function saveFileAjaxAction(
    $parentId, $parentType, $objectType, $validMimeTypes = array(), $extBlackList = array()
    )
    {
        ...
        if ($this->getRequest()->isMethod('POST')) {
            ...
            try {
                $this->processFile(
                 $fileBeingUploaded, $parentId, $parentType, $objectType, $validMimeTypes, $extBlackList
                );
    ...
    $fileBeingUploaded, $parentId, $parentType, $objectType, $validMimeTypes = array(), $extBlackList = array()
    )
    {
        ...
        if ($fileBeingUploaded->getError() == 1) {
            $message = $this->getTranslator()
             ->trans(
             'File is too large, please retry with a file having a size less than %size%.', array('%size%' => ini_get('upload_max_filesize')), 'core'
            );
        ...
        if (!empty($validMimeTypes)) {
            ...
            if (!isset($validMimeTypes[$mimeType])) {
                $message = $this->getTranslator()
                 ->trans(
                 'Only files having the following mime type are allowed: %types%', ['%types%' => implode(', ', array_keys($validMimeTypes))]
                );
            } else {
    ...
    {
        $newHook = new FilterConfiguratorHook();
        $newHook->save();

        return $this->generateRedirect(URL::getInstance()->absoluteUrl("/admin/module/FilterConfigurator"));
    }

    public function deleteHookIdConfigurator($id)
    {

        $hookFilterConfigurator = FilterConfiguratorHookQuery::create()
         ->findOneById($id);

        if ($hookFilterConfigurator) {

            $module   = ModuleQuery::create()
             ->findOneByCode("FilterConfigurator");
            $moduleId = $module->getId();

            $query = ModuleHookQuery::create()
             ->filterByModuleId($moduleId)
             ->findOneByHookId($hookFilterConfigurator->getHookId());

            if ($query) {
                $query->delete();
            }

            $hookFilterConfigurator->delete();
        }
    ...
    public function editHookIdConfigurator()
    {
        ...
        if (null !== $response = $this->checkAuth(AdminResources::MODULE, 'FilterConfigurator', AccessManager::UPDATE)) {
            return $response;
        }

        $configurator = $this->createForm("filter.configurator.hook.form");

        try {
            $form           = $this->validateForm($configurator, "POST");
            $data           = $form->getData();
            $idConfigurator = FilterConfiguratorQuery::create()
             ->findOneById($data['filter_configurator_id']);

            if ($idConfigurator) {
                $hookFilterConfigurator = FilterConfiguratorHookQuery::create()
                 ->findOneById($this->getRequest()->request->get("hook_key_id"));

                $oldHookId = $hookFilterConfigurator->getHookId();

                $hookFilterConfigurator->setFilterConfiguratorId($idConfigurator->getId());
                $hookFilterConfigurator->setHookCode($data["hook_code"]);
                $hookFilterConfigurator->setHookId($data["hook_id"]);
                $hookFilterConfigurator->save();
            ...
            $url = '/admin/module/FilterConfigurator';
        ...
        $this->setupFormErrorContext($this->getTranslator()
          ->trans("Filter Configuration", [], FilterConfigurator::DOMAIN_NAME), $error_msg, $configurator, $ex);
        ...
        if (Version::test(Thelia::THELIA_VERSION, '2.2', false, "<")) {
            return $this->render('module-configure', [
              'module_code' => 'FilterConfigurator'
            ]);
        } else {
            return $this->generateRedirect(URL::getInstance()->absoluteUrl('/admin/module/FilterConfigurator'));
        }
    ...
    protected function saveModuleHook($moduleId, $hookId, $oldHookId)
    {
        ...
        if ($hookId) {
            ...
            $moduleHook->setClassname("feature.configurator");
            $moduleHook->setMethod("configuratorFiltersSelection");
