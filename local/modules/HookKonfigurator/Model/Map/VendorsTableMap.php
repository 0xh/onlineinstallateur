...
class VendorsTableMap extends TableMap
{
     ...
     */
    const CLASS_NAME = 'HookKonfigurator.Model.Map.VendorsTableMap';
     ...
     */
    const OM_CLASS = '\\HookKonfigurator\\Model\\Vendors';

    /**
     * A class that can be returned by this tableMap
     */
    const CLASS_DEFAULT = 'HookKonfigurator.Model.Vendors';
    ...
    protected static $fieldNames = array (
        ...
        self::TYPE_COLNAME       => array(VendorsTableMap::ID, VendorsTableMap::NAME, ),
    ...
    protected static $fieldKeys = array (
        ...
        self::TYPE_COLNAME       => array(VendorsTableMap::ID => 0, VendorsTableMap::NAME => 1, ),
    ...
    public function initialize()
    {
        ...
        $this->setClassName('\\HookKonfigurator\\Model\\Vendors');
    ...
    {
        if (null === $alias) {
            $criteria->addSelectColumn(VendorsTableMap::ID);
            $criteria->addSelectColumn(VendorsTableMap::NAME);
        } else {
     ...
     */
     public static function doDelete($values, ConnectionInterface $con = null)
     {
        ...
        if ($values instanceof Criteria) {
            // rename for clarity
            $criteria = $values;
        } elseif ($values instanceof \HookKonfigurator\Model\Vendors) { // it's a model object
            // create criteria based on pk values
            $criteria = $values->buildPkeyCriteria();
        } else { // it's a primary key, or an array of pks
            $criteria = new Criteria(VendorsTableMap::DATABASE_NAME);
            $criteria->add(VendorsTableMap::ID, (array) $values, Criteria::IN);
        }

        $query = VendorsQuery::create()->mergeWith($criteria);

        if ($values instanceof Criteria) { VendorsTableMap::clearInstancePool();
        } elseif (!is_object($values)) { // it's a primary key, or an array of pks
            foreach ((array) $values as $singleval) { VendorsTableMap::removeInstanceFromPool($singleval);
            }
        }
    ...
    public static function doInsert($criteria, ConnectionInterface $con = null)
    {
        ...
        if ($criteria->containsKey(VendorsTableMap::ID) && $criteria->keyContainsValue(VendorsTableMap::ID) ) {
