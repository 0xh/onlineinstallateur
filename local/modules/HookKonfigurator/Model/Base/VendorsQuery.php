...
use \Exception;
use \PDO;
use HookKonfigurator\Model\Vendors as ChildVendors;
 ...
 *
 *
 * @method     ChildVendorsQuery orderById($order = Criteria::ASC) Order by the id column
 * @method     ChildVendorsQuery orderByName($order = Criteria::ASC) Order by the name column
 *
 * @method     ChildVendorsQuery groupById() Group by the id column
 ...
 *
 * @method     ChildVendors findOneById(int $id) Return the first ChildVendors filtered by the id column
 * @method     ChildVendors findOneByName(string $name) Return the first ChildVendors filtered by the name column
 *
 * @method     array findById(int $id) Return ChildVendors objects filtered by the id column
 * @method     array findByName(string $name) Return ChildVendors objects filtered by the name column
 *
 */
abstract class VendorsQuery extends ModelCriteria
{
    ...
    /**
     * Initializes internal state of \HookKonfigurator\Model\Base\VendorsQuery object.
     *
     * @param     string $dbName The database name
     * @param     string $modelName The phpName of a model, e.g. 'Book'
     * @param     string $modelAlias The alias for the model in this query, e.g. 'b'
     */
    public function __construct($dbName = 'thelia', $modelName = '\\HookKonfigurator\\Model\\Vendors', $modelAlias = null)
    {
    ...
    {
        if ($criteria instanceof \HookKonfigurator\Model\VendorsQuery) {
            return $criteria;
        }
        $query = new \HookKonfigurator\Model\VendorsQuery();
        if (null !== $modelAlias) {
    ...
    public function findPk($key, $con = null)
    {
        ...
        if ((null !== ($obj = VendorsTableMap::getInstanceFromPool((string) $key))) && !$this->formatter) {
    ...
    protected function findPkSimple($key, $con)
    {
        ...
        if ($row = $stmt->fetch(\PDO::FETCH_NUM)) {
            ...
            $obj->hydrate($row);
            VendorsTableMap::addInstanceToPool($obj, (string) $key);
        }
        $stmt->closeCursor();
     ...
     * @param     mixed $key Primary key to use for the query
     *
     * @return ChildVendorsQuery The current query, for fluid interface
     */
    public function filterByPrimaryKey($key)
    {

        return $this->addUsingAlias(VendorsTableMap::ID, $key, Criteria::EQUAL);
    }
    ...
    public function filterByPrimaryKeys($keys)
    {
        ...
        return $this->addUsingAlias(VendorsTableMap::ID, $keys, Criteria::IN);
    }
    ...
    public function filterById($id = null, $comparison = null)
    {
        if (is_array($id)) {
            $useMinMax = false;
            if (isset($id['min'])) {
                $this->addUsingAlias(VendorsTableMap::ID, $id['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($id['max'])) {
                $this->addUsingAlias(VendorsTableMap::ID, $id['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
        ...
        return $this->addUsingAlias(VendorsTableMap::ID, $id, $comparison);
    }
    ...
    public function filterByName($name = null, $comparison = null)
    {
        ...
        return $this->addUsingAlias(VendorsTableMap::NAME, $name, $comparison);
    }
    ...
    {
        if ($vendors) {
            $this->addUsingAlias(VendorsTableMap::ID, $vendors->getId(), Criteria::NOT_EQUAL);
        }
    ...
    /**
     * Performs a DELETE on the database, given a ChildVendors or Criteria object OR a primary key value.
     *
     * @param mixed               $values Criteria or ChildVendors object or primary key or array of primary keys
