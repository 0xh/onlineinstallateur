...
include_once dirname(__DIR__) . '/../MarketplaceWebService/Model/GetReportRequest.php';
 ...
 */
function invokeGetReport(MarketplaceWebService_Interface $service, $request, $serviceUrl = "")
{
    try {
        $response = $service->getReport($request);

        $getContents = stream_get_contents($request->getReport());
        $arrTest     = explode("\n", $getContents);
        $list        = array();

        foreach ($arrTest as $value) {
            $res = explode("	", $value);
            if (isset($res[2]) && (strpos($serviceUrl, '.fr') == false)) {
                array_push($list, array("pr"       => $res[0], 'idAmazon' => $res[2], 'sku'      => $res[3], 'price'    => $res[4],
                 'quantity' => $res[5],
                 'asin'     => $res[16]));
            } else {
                if (isset($res[1])) {
                    array_push($list, array("pr"       => $res[0], 'idAmazon' => $res[1], 'sku'      => $res[2], 'price'    => $res[3],
                     'quantity' => $res[4],
                     'asin'     => @$res[11]));
//                    echo "<pre>";
//                    var_dump($res[0]);
