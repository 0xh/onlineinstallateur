 ...
 ***********************************************************************/
 ...
 $productService = invokeGetServiceStatus($service, $request);
  ...
  */
  ...
  function invokeGetServiceStatus(MarketplaceWebServiceProducts_Interface $service, $request)
  {
      try {
        ...
        $xml = $dom->saveXML();
        $prdData = new SimpleXMLElement($xml);
        $array = json_encode($prdData, TRUE);
        $result = json_decode($array);
        if ($result) {
            foreach ($result as $prd) {
                
                $productService['Status'] = $prd->Status;
                $productService['Timestamp'] = $prd->Timestamp;
                $productService['Messages'] = "Not have Messages";
                break;
            }
        } else {
            $productService['Status'] = "";
            $productService['Timestamp'] = "";
            $productService['Messages'] = 'Error decoding json';
        }
