      ...
      try {
        ...
        if ($query_features !== null) {
          foreach ($query_features as $feature) {                                    
           $arrFeautures[] = 
            array("feature_title" => (string) $feature->getVirtualColumn('feature_title'),
        ...
        $result = array(
          ...
          'is_active' => 1,
          'created_at'=>  $item->getVirtualColumn("created_at") <> NULL ? date("Y-m-d H:m:s",strtotime((string)$item->getVirtualColumn("created_at"))) :  date("Y-m-d H:m:s",strtotime((string)$item->getVirtualColumn("update_at"))),
          'update_at' =>  date("Y-m-d H:m:s",strtotime((string)$item->getVirtualColumn("update_at"))),
