...
class MetaDataTableMap extends TableMap
{
     ...
     */
    const CLASS_NAME = 'Thelia.Model.Map.MetaDataTableMap';
     ...
     */
    const OM_CLASS = '\\Thelia\\Model\\MetaData';

    /**
     * A class that can be returned by this tableMap
     */
    const CLASS_DEFAULT = 'Thelia.Model.MetaData';
     ...
     */
    const META_KEY = 'meta_data.META_KEY';

    /**
     * the column name for the ELEMENT_KEY field
     */
    const ELEMENT_KEY = 'meta_data.ELEMENT_KEY';

    /**
     * the column name for the ELEMENT_ID field
     */
    const ELEMENT_ID = 'meta_data.ELEMENT_ID';

    /**
     * the column name for the IS_SERIALIZED field
     */
    const IS_SERIALIZED = 'meta_data.IS_SERIALIZED';
    ...
    {
        // attributes
        $this->setName('meta_data');
        $this->setPhpName('MetaData');
        $this->setClassName('\\Thelia\\Model\\MetaData');
        $this->setPackage('Thelia.Model');
        $this->setUseIdGenerator(true);
        // columns
        $this->addPrimaryKey('ID', 'Id', 'INTEGER', true, null, null);
        $this->addColumn('META_KEY', 'MetaKey', 'VARCHAR', true, 100, null);
        $this->addColumn('ELEMENT_KEY', 'ElementKey', 'VARCHAR', true, 100, null);
    ...
    {
        if (null === $alias) {
            $criteria->addSelectColumn(MetaDataTableMap::ID);
            $criteria->addSelectColumn(MetaDataTableMap::META_KEY);
            $criteria->addSelectColumn(MetaDataTableMap::ELEMENT_KEY);
     ...
     */
     public static function doDelete($values, ConnectionInterface $con = null)
     {
        ...
        if ($values instanceof Criteria) {
            // rename for clarity
            $criteria = $values;
        } elseif ($values instanceof \Thelia\Model\MetaData) { // it's a model object
            // create criteria based on pk values
            $criteria = $values->buildPkeyCriteria();
        } else { // it's a primary key, or an array of pks
            $criteria = new Criteria(MetaDataTableMap::DATABASE_NAME);
            $criteria->add(MetaDataTableMap::ID, (array) $values, Criteria::IN);
        }

        $query = MetaDataQuery::create()->mergeWith($criteria);

        if ($values instanceof Criteria) { MetaDataTableMap::clearInstancePool();
        } elseif (!is_object($values)) { // it's a primary key, or an array of pks
            foreach ((array) $values as $singleval) { MetaDataTableMap::removeInstanceFromPool($singleval);
            }
        }
