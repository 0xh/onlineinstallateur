...
abstract class MetaData implements ActiveRecordInterface
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\Thelia\\Model\\Map\\MetaDataTableMap';
    ...
    /**
     * The value for the is_serialized field.
     * @var        boolean
     */
    ...
    /**
     * Initializes internal state of Thelia\Model\Base\MetaData object.
     */
     ...
     *
     * @return MetaData The current object, for fluid interface
     */
    ...
    /**
     * Get the [is_serialized] column value.
     *
     * @return   boolean
     */
    ...
    public function setId($v)
    {
        ...
        if ($this->id !== $v) {
            $this->id = $v;
            $this->modifiedColumns[MetaDataTableMap::ID] = true;
        }
    ...
    public function setMetaKey($v)
    {
        ...
        if ($this->meta_key !== $v) {
            $this->meta_key = $v;
            $this->modifiedColumns[MetaDataTableMap::META_KEY] = true;
        }
    ...
    public function setElementKey($v)
    {
        ...
        if ($this->element_key !== $v) {
            $this->element_key = $v;
            $this->modifiedColumns[MetaDataTableMap::ELEMENT_KEY] = true;
        }
    ...
    public function setElementId($v)
    {
        ...
        if ($this->element_id !== $v) {
            $this->element_id = $v;
            $this->modifiedColumns[MetaDataTableMap::ELEMENT_ID] = true;
        }
    ...
    /**
     * Sets the value of the [is_serialized] column.
     ...
     * @return   \Thelia\Model\MetaData The current object (for fluent API support)
     */
    public function setIsSerialized($v)
    {
        if ($v !== null) {
            if (is_string($v)) {
        ...
        if ($this->is_serialized !== $v) {
            $this->is_serialized = $v;
            $this->modifiedColumns[MetaDataTableMap::IS_SERIALIZED] = true;
        }
    ...
    public function setValue($v)
    {
        ...
        if ($this->value !== $v) {
            $this->value = $v;
            $this->modifiedColumns[MetaDataTableMap::VALUE] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 0 + $startcol : MetaDataTableMap::translateFieldName('Id', TableMap::TYPE_PHPNAME, $indexType)];
            $this->id = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : MetaDataTableMap::translateFieldName('MetaKey', TableMap::TYPE_PHPNAME, $indexType)];
            $this->meta_key = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : MetaDataTableMap::translateFieldName('ElementKey', TableMap::TYPE_PHPNAME, $indexType)];
            $this->element_key = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 3 + $startcol : MetaDataTableMap::translateFieldName('ElementId', TableMap::TYPE_PHPNAME, $indexType)];
            $this->element_id = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 4 + $startcol : MetaDataTableMap::translateFieldName('IsSerialized', TableMap::TYPE_PHPNAME, $indexType)];
            $this->is_serialized = (null !== $col) ? (boolean) $col : null;
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(MetaDataTableMap::ID)) {
            $modifiedColumns[':p' . $index++]  = '`ID`';
        }
        if ($this->isColumnModified(MetaDataTableMap::META_KEY)) {
            $modifiedColumns[':p' . $index++]  = '`META_KEY`';
        }
        if ($this->isColumnModified(MetaDataTableMap::ELEMENT_KEY)) {
            $modifiedColumns[':p' . $index++]  = '`ELEMENT_KEY`';
        }
        if ($this->isColumnModified(MetaDataTableMap::ELEMENT_ID)) {
            $modifiedColumns[':p' . $index++]  = '`ELEMENT_ID`';
        }
        if ($this->isColumnModified(MetaDataTableMap::IS_SERIALIZED)) {
            $modifiedColumns[':p' . $index++]  = '`IS_SERIALIZED`';
        }
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    ...
                    case '`ELEMENT_ID`':
                        ...
                        break;
                    case '`IS_SERIALIZED`':
                        $stmt->bindValue($identifier, (int) $this->is_serialized, PDO::PARAM_INT);
                        break;
                    case '`VALUE`':
    ...
    public function toArray($keyType = TableMap::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array())
    {
        ...
        $result = array(
            $keys[0] => $this->getId(),
            $keys[1] => $this->getMetaKey(),
            $keys[2] => $this->getElementKey(),
    ...
    public function fromArray($arr, $keyType = TableMap::TYPE_PHPNAME)
    {
        ...
        if (array_key_exists($keys[1], $arr)) $this->setMetaKey($arr[$keys[1]]);
        if (array_key_exists($keys[2], $arr)) $this->setElementKey($arr[$keys[2]]);
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(MetaDataTableMap::ID)) $criteria->add(MetaDataTableMap::ID, $this->id);
        if ($this->isColumnModified(MetaDataTableMap::META_KEY)) $criteria->add(MetaDataTableMap::META_KEY, $this->meta_key);
        if ($this->isColumnModified(MetaDataTableMap::ELEMENT_KEY)) $criteria->add(MetaDataTableMap::ELEMENT_KEY, $this->element_key);
    ...
    {
        $criteria = new Criteria(MetaDataTableMap::DATABASE_NAME);
        $criteria->add(MetaDataTableMap::ID, $this->id);
     ...
     * objects.
     *
     * @param      object $copyObj An object of \Thelia\Model\MetaData (or compatible) type.
     ...
     *
     * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return                 \Thelia\Model\MetaData Clone of current object.
     * @throws PropelException
     */
