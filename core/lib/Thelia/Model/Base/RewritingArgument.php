...
abstract class RewritingArgument implements ActiveRecordInterface
{
    /**
     * TableMap class name
     */
    ...
    /**
     * The value for the rewriting_url_id field.
     * @var        int
     */
    protected $rewriting_url_id;

    /**
     * The value for the parameter field.
     * @var        string
     */
    protected $parameter;
    ...
    /**
     * Compares this with another <code>RewritingArgument</code> instance.  If
     * <code>obj</code> is an instance of <code>RewritingArgument</code>, delegates to
     * <code>equals(RewritingArgument)</code>.  Otherwise, returns <code>false</code>.
     *
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 0 + $startcol : RewritingArgumentTableMap::translateFieldName('RewritingUrlId', TableMap::TYPE_PHPNAME, $indexType)];
            $this->rewriting_url_id = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : RewritingArgumentTableMap::translateFieldName('Parameter', TableMap::TYPE_PHPNAME, $indexType)];
            $this->parameter = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : RewritingArgumentTableMap::translateFieldName('Value', TableMap::TYPE_PHPNAME, $indexType)];
            $this->value = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 3 + $startcol : RewritingArgumentTableMap::translateFieldName('CreatedAt', TableMap::TYPE_PHPNAME, $indexType)];
            if ($col === '0000-00-00 00:00:00') {
                $col = null;
            }
            $this->created_at = (null !== $col) ? PropelDateTime::newInstance($col, null, '\DateTime') : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 4 + $startcol : RewritingArgumentTableMap::translateFieldName('UpdatedAt', TableMap::TYPE_PHPNAME, $indexType)];
            if ($col === '0000-00-00 00:00:00') {
                $col = null;
            }
            ...
            return $startcol + 5; // 5 = RewritingArgumentTableMap::NUM_HYDRATE_COLUMNS.

        } catch (Exception $e) {
            throw new PropelException("Error populating \Thelia\Model\RewritingArgument object", 0, $e);
        }
    }
    ...
    {
        if ($this->aRewritingUrl !== null && $this->rewriting_url_id !== $this->aRewritingUrl->getId()) {
            $this->aRewritingUrl = null;
        }
    } // ensureConsistency
    ...
    public function reload($deep = false, ConnectionInterface $con = null)
    {
        ...
        $dataFetcher = ChildRewritingArgumentQuery::create(null, $this->buildPkeyCriteria())->setFormatter(ModelCriteria::FORMAT_STATEMENT)->find($con);
    ...
    public function delete(ConnectionInterface $con = null)
    {
        ...
        try {
            $deleteQuery = ChildRewritingArgumentQuery::create()
    ...
    public function save(ConnectionInterface $con = null)
    {
        ...
        try {
            ...
            if ($isInsert) {
                ...
                // timestampable behavior
                if ($this->isModified() && !$this->isColumnModified(RewritingArgumentTableMap::UPDATED_AT)) {
                    $this->setUpdatedAt(time());
                }
            }
            if ($ret) {
                ...
                $this->postSave($con);
                RewritingArgumentTableMap::addInstanceToPool($this);
            } else {
                $affectedRows = 0;
            }
            $con->commit();
    ...
    protected function doSave(ConnectionInterface $con)
    {
        ...
        if (!$this->alreadyInSave) {
            ...
            if ($this->aRewritingUrl !== null) {
                if ($this->aRewritingUrl->isModified() || $this->aRewritingUrl->isNew()) {
                    $affectedRows += $this->aRewritingUrl->save($con);
                }
                $this->setRewritingUrl($this->aRewritingUrl);
            }
    ...
    {
        $pos = RewritingArgumentTableMap::translateFieldName($name, $type, TableMap::TYPE_NUM);
    ...
    {
        if (isset($alreadyDumpedObjects['RewritingArgument'][serialize($this->getPrimaryKey())])) {
            return '*RECURSION*';
        }
        $alreadyDumpedObjects['RewritingArgument'][serialize($this->getPrimaryKey())] = true;
        $keys = RewritingArgumentTableMap::getFieldNames($keyType);
        $result = array(
            $keys[0] => $this->getRewritingUrlId(),
            $keys[1] => $this->getParameter(),
        ...
        if ($includeForeignObjects) {
            if (null !== $this->aRewritingUrl) {
                $result['RewritingUrl'] = $this->aRewritingUrl->toArray($keyType, $includeLazyLoadColumns,  $alreadyDumpedObjects, true);
            }
        }
    ...
    public function fromArray($arr, $keyType = TableMap::TYPE_PHPNAME)
    {
        ...
        if (array_key_exists($keys[0], $arr)) $this->setRewritingUrlId($arr[$keys[0]]);
        if (array_key_exists($keys[1], $arr)) $this->setParameter($arr[$keys[1]]);
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(RewritingArgumentTableMap::REWRITING_URL_ID)) $criteria->add(RewritingArgumentTableMap::REWRITING_URL_ID, $this->rewriting_url_id);
        if ($this->isColumnModified(RewritingArgumentTableMap::PARAMETER)) $criteria->add(RewritingArgumentTableMap::PARAMETER, $this->parameter);
    ...
    {
        $criteria = new Criteria(RewritingArgumentTableMap::DATABASE_NAME);
        $criteria->add(RewritingArgumentTableMap::REWRITING_URL_ID, $this->rewriting_url_id);
        $criteria->add(RewritingArgumentTableMap::PARAMETER, $this->parameter);
        $criteria->add(RewritingArgumentTableMap::VALUE, $this->value);
    ...
    {
        $pks = array();
        $pks[0] = $this->getRewritingUrlId();
        $pks[1] = $this->getParameter();
        $pks[2] = $this->getValue();
    ...
    {
        $this->setRewritingUrlId($keys[0]);
        $this->setParameter($keys[1]);
        $this->setValue($keys[2]);
    }
    ...
    {
        $copyObj->setRewritingUrlId($this->getRewritingUrlId());
    ...
    {
        if ($v === null) {
            $this->setRewritingUrlId(NULL);
        } else {
            $this->setRewritingUrlId($v->getId());
        }

        $this->aRewritingUrl = $v;
    ...
    {
        return (string) $this->exportTo(RewritingArgumentTableMap::DEFAULT_STRING_FORMAT);
    }
