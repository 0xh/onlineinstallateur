...
var pseManager = function(a) {
    ...
    function c() {
        var b, c = null,
            ...
            h = null,
            i = [];
        if (m.pseId = m.id.val(), PSE_COUNT > 1)
            if (m.useFallback = j(), m.useFallback) {
                a("#pse-options .option-option").remove();
                for (c in PSE) {
                    if (h = PSE[c].combinations, i = [], void 0 !== h)
                        for (b = 0; b < h.length; b++) i.push(PSE_COMBINATIONS_VALUE[h[b]][0]);
    ...
    function d(a) {
        var b, c = null;
        if (c = PSE[a || m.pseId], void 0 !== c)
            if (m.useFallback) m.fallbak.val(c.id);
            else if (void 0 !== c)
            for (var d = 0; d < c.combinations.length; d++) b = c.combinations[d], m.options[PSE_COMBINATIONS_VALUE[b][1]].val(c.combinations[d])
    }
    ...
    function g() {
        ...
        if (void 0 !== c) {
            ...
            c.isPromo ? m.product.addClass("product--is-promo") : m.product.removeClass("product--is-promo"), c.isNew ? m.product.addClass("product--is-new") : m.product.removeClass("product--is-new"), c.quantity > 0 || !PSE_CHECK_AVAILABILITY ? (h(!0), parseInt(m.quantity.val()) > c.quantity && m.quantity.val(c.quantity), PSE_CHECK_AVAILABILITY ? m.quantity.attr("max", c.quantity) : (m.quantity.attr("max", PSE_DEFAULT_AVAILABLE_STOCK), m.quantity.val("1"))) : h(!1), c.isPromo ? (m.priceOld.html(c.price), m.price.html(c.promo)) : (m.priceOld.html(""), m.price.html(c.price))
        } else h(!1)
    }

    function h(a) {
        a ? (m.availability.removeClass("out-of-stock").addClass("in-stock").attr("href", "http://schema.org/InStock"), m.submit.prop("disabled", !1)) : (m.availability.removeClass("in-stock").addClass("out-of-stock").attr("href", "http://schema.org/OutOfStock"), m.submit.prop("disabled", !0))
    }

    function i(a) {
        var b, c, d, e, f, g = null;
        for (g in PSE)
            if (b = g, c = PSE[g].combinations, void 0 !== c) {
                for (d = 0; d < a.length; d++) {
                    for (f = !1, e = 0; e < c.length; e++)
                        if (a[d] == c[e]) {
                            f = !0;
                            break
                        }
    ...
    function j() {
        var a, b, c = null,
            ...
            e = 0;
        for (c in PSE)
            if (a = PSE[c].combinations, void 0 !== a) {
...
//                b.trigger("mouseleave.subnav")
        ...
        }), a(document).on("click.confirm", "[data-confirm]", function() {
            var c = a(this),
                d = c.attr("href"),
                e = c.attr("data-confirm-callback"),
                f = "" !== c.attr("data-confirm") ? c.attr("data-confirm") : "Are you sure?";
            return bootbox.confirm(f, function(f) {
                if (f)
                    if (e && a.isFunction(b[e])) b[e](c);
                    else if (d) window.location.href = d;
                else {
        ...
        g.on("change.account", ":radio", function() {
            ...
            a(".item", this).size() > 1 && a("#product-thumbnails", this).carousel({
                interval: !1
        ...
        }), a("#form-contact, #form-register, #form-address,#form-services, #form-personal-data, #form-heizungangebot").validate({
...
function(a) {
    a(document).ready(function() {
        var b = function() {
            return initialize = function(b) {
                var c = {};
                c.state = a(b), c.stateId = c.state.val(), c.country = a(c.state.data("thelia-country")), c.countryId = c.country.val(), c.block = a(c.state.data("thelia-toggle")), c.states = c.state.children().clone(), c.state.children().remove();
                var d = function() {
                    var b = c.country.val(),
                        d = c.state.val(),
                        e = !1;
                    null !== d && "" !== d && (c.stateId = d), c.state.children().remove(), c.states.each(function() {
                        var d = a(this);
                        d.data("country") == b && (d.appendTo(c.state), e = !0)
                    }), e ? (c.state.val(c.stateId), c.block.removeClass("hidden")) : c.block.addClass("hidden")
                };
                c.country.on("change", d), d()
            }, {
                init: function() {
                    a("[data-thelia-state]").each(function() {
                        initialize(this)
                    })
                }
            }
...
$( "#form-product-details .wish-list-button a" ).click(function(e) {

  var id = $(this).parent('.wish-list-button').find('.wish-list-product-id').val();
  if($(this).hasClass("in-wish-list")) {
...
function addWishList(object, id) {
    $.ajax({
        url : "/wishlist/add/"+id,
        type: "GET"
    }).done(function(data) {
    	object.addClass("in-wish-list");
    	object.attr("href", "/wishlist/remove/"+id);
    	object.find('span').text(object.parent('.wish-list-button').find('.wish-list-text-remove').val());
    	object.find('i').removeClass('fa-heart-o').addClass('fa-heart');
    	if($('.wish .badge').length){ 
    		$('.wish .badge').text(parseInt($('.wish .badge').text()) + 1);
		}
    	if($('.wish-content p').length) {
    		if($('.wish .badge').text() == '1') {
    			message = $('.wish-content .wishlist_message_item').text().replace('%nr',  $('.wish .badge').text());
    		}
    		else {
    			message = $('.wish-content .wishlist_message_items').text().replace('%nr',  $('.wish .badge').text());
    		}
    		
    		$('.wish-content p').empty().text(message);
		}
    });
}

function removeWishList(object, id) {
    $.ajax({
        url : "/wishlist/remove/"+id,
        type: "GET"
    }).done(function(data) {
    	object.removeClass("in-wish-list");
    	object.attr("href", "/wishlist/add/"+id);
    	object.find('span').text(object.parent('.wish-list-button').find('.wish-list-text-add').val());
    	object.find('i').removeClass('fa-heart').addClass('fa-heart-o');
    	if($('.wish .badge').length){ 
    		$('.wish .badge').text(parseInt($('.wish .badge').text()) - 1);
		}
    	if($('.wish-content p').length){ 
    	
    		if($('.wish .badge').text() == '0') {
    			message = $('.wish-content .wishlist_message_no_items').text();
    		}
    		else if($('.wish .badge').text() == '1') {
    			message = $('.wish-content .wishlist_message_item').text().replace('%nr',  $('.wish .badge').text());
    		}
    		else {
    			message = $('.wish-content .wishlist_message_items').text().replace('%nr',  $('.wish .badge').text());
