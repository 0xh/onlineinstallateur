...
class pjAdmin extends pjAppController
{
	...
	private function pjActionGetDashboard($isoDate)
	{
		...
		$employee_arr = pjEmployeeModel::factory()
			->select("t1.*, t2.content AS `name`, (SELECT GROUP_CONCAT(CONCAT_WS('|', TS.id,TS.total, TS.before) SEPARATOR '~:~') FROM `".pjServiceModel::factory()->getTable()."` AS TS WHERE TS.id IN (SELECT TMS.service_id FROM `".pjEmployeeServiceModel::factory()->getTable()."` AS TMS WHERE TMS.employee_id = t1.id ) ) AS services")
