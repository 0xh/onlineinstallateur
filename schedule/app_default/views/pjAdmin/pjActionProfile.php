...
if (isset($tpl['status']))
{
	...
	<div class="ui-tabs ui-widget ui-widget-content ui-corner-all b10">
		<ul class="ui-tabs-nav ui-helper-reset ui-helper-clearfix ui-widget-header ui-corner-all">
			...
			{
				?><li class="ui-state-default ui-corner-top<?php echo $active; ?>"><a href="<?php echo $_SERVER['PHP_SELF']; ?>?controller=pjAdminTime&amp;action=pjActionUpdateCustom&amp;type=employee&amp;foreign_id=<?php echo $employee_id; ?>&amp;id=<?php echo @$tpl['arr']['id']; ?>"><?php __('time_update_custom'); ?></a></li><?php
			}
			?>
		</ul>
	</div>
	...
	<form action="<?php echo $_SERVER['PHP_SELF']; ?>?controller=pjAdmin&amp;action=pjActionProfile" method="post" id="frmUpdateProfile" class="form pj-form" enctype="multipart/form-data" autocomplete="off">
		...
		<p style="position: relative">
			...
			<span class="pj-form-field-custom pj-form-field-custom-before">
				...
				<input type="text" name="email" id="email" class="pj-form-field required email w200" value="<?php echo pjSanitize::html($tpl['arr']['email']); ?>" autocomplete="off" />
			</span>
		...
		<p>
			...
			<span class="pj-form-field-custom pj-form-field-custom-before">
				...
				<input type="password" name="password" id="password" class="pj-form-field required w200" value="password" autocomplete="off" />
			</span>
		</p>
		<p>
		...
		<p>
			...
			<span class="float_left block">
				...
				if (!empty($tpl['arr']['avatar']) && is_file($tpl['arr']['avatar']))
				{
					...
					<span class="boxNoAvatar" style="display: none"><img src="<?php echo PJ_INSTALL_URL . PJ_IMG_PATH; ?>backend/professional.gif" alt="" /><br /></span>
					<?php
				} else {
					?><img src="<?php echo PJ_INSTALL_URL . PJ_IMG_PATH; ?>backend/professional.gif" alt="" /><br /><?php
				}
				?>
