...
list($n, $j) = explode("-", date("n-j", strtotime($_GET['date'])));
				...
				<ul class="list-group pjAsListElements pjAsEmployees">
					...
					if(!empty($tpl['employee_arr']))
					{
						...
						foreach($tpl['employee_arr'] as $k => $employee)
						{
							...
							{
								$slug = sprintf("%s/%s/%s/%s/%s-%u.html", 'Employee', $year, $month, $day, pjAppController::friendlyURL($employee['name']), $employee['id']);
							} else {
								$slug = NULL;
							}
							?>
							<li class="list-group-item pjAsListElement pjAsEmployee">
								<div class="row">
									<div class="col-lg-3 col-md-3 col-sm-3 col-xs-12">
										<a href="#" class="pjAsEmployeeAppointment" data-iso="<?php echo $_GET['date']; ?>" data-eid="<?php echo $employee['id']; ?>" data-slug="<?php echo $slug;?>">
									...
									<div class="col-lg-9 col-md-9 col-sm-9 col-xs-12">
										...
										{ 
											?>
											<a href="#" class="btn btn-default pjAsBtn pjAsBtnPrimary pjAsEmployeeAppointment" data-iso="<?php echo $_GET['date']; ?>" data-eid="<?php echo $employee['id']; ?>" data-slug="<?php echo $slug;?>"><?php __('front_make_appointment');?></a>
											<?php
										} 
										?>
								...
								{
									if(isset($_GET['employee_id']) && $employee['id'] == $_GET['employee_id'])
									{
										list($year, $month, $day) = explode("-", $_GET['date']);
										...
										{ 
											...
											<div class="form-horizontal asEmployeeInfo">
												<form class="pjAsAddToCartForm" action="" method="post">
													<div class="form-group">
														...
														<div class="col-lg-9 col-md-9 col-sm-9 col-sx-12">
															...
															{ 
																...
																{
																	?>
																	<select name="service_id" class="form-control pjAsSelectorService" data-iso="<?php echo $_GET['date'];?>" data-employee_id="<?php echo $employee['id'];?>" >
																		...
																		foreach($tpl['service_arr'] as $service)
																		{
																			if ((int) $tpl['option_arr']['o_seo_url'] === 1)
																			{
																				$slug = sprintf("%s/%s/%s/%s/%s-%u/%s-%u.html", 'Employee', $year, $month, $day, pjAppController::friendlyURL($employee['name']), $employee['id'], pjAppController::friendlyURL($service['name']), $service['id']);
																			} else {
																				$slug = NULL;
																			}
																			if(isset($tpl['service']))
																	...
																	<label class="form-control-static"><?php echo $tpl['service_arr'][0]['name'];?></label>
																	<?php
																}
															}else{
																?><p class="form-control-static"><?php echo $tpl['service']['name'];?></p><?php
															} 
															?>
														</div>
													</div>
													...
													<input type="hidden" name="date" value="<?php echo date($tpl['option_arr']['o_date_format'], strtotime($_GET['date']));?>" />
