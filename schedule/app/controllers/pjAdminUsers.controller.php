...
class pjAdminUsers extends pjAdmin
{
	...
	public function pjActionCreate()
	{
		...
		if ($this->isAdmin())
		{
			if (isset($_POST['user_create']))
			{
				...
				$data['is_notify'] = isset($_POST['is_notify']) ? 'T' : 'F';
				unset($_POST['is_notify']);
				$data = array_merge($_POST, $data);
				
				$required = array('email', 'password', 'name', 'status');
	...
	public function pjActionDeleteUser()
	{
		...
		if ($this->isXHR() && $this->isLoged() && $this->isAdmin())
		{
			if (isset($_GET['id']) && !in_array($_GET['id'], array(1, $this->getUserId())))
			{
				...
				pjAppController::jsonResponse(array('status' => 'ERR', 'code' => 101, 'text' => 'Missing or invalid data.'));
			}
		}
		exit;
	}
		...
		if ($this->isXHR() && $this->isLoged() && $this->isAdmin())
		{
			if (isset($_POST['record']) && !empty($_POST['record']))
			{
				...
				pjMultiLangModel::factory()
					...
					->where('foreign_id !=', 1)
					->whereIn('foreign_id', $_POST['record'])->eraseAll();
				pjAppController::jsonResponse(array('status' => 'OK', 'code' => 200, 'text' => 'User(s) has been deleted.'));
			}
	...
	public function pjActionGetUser()
	{
		...
		if ($this->isXHR() && $this->isLoged() && $this->isAdmin())
		{
			...
			$data = $pjUserModel->select('t1.id, t1.email, t1.name, DATE(t1.created) AS created, t1.status')
	...
	public function pjActionSaveUser()
	{
		...
		if ($this->isXHR() && $this->isLoged() && $this->isAdmin())
		{
			if (isset($_POST['column']) && isset($_POST['value']) && !empty($_POST['column']))
			{
				if ($_POST['column'] == 'status' && $_GET['id'] == $this->getUserId())
				{
					pjAppController::jsonResponse(array('status' => 'ERR', 'code' => 101, 'text' => 'Cannot change your own status.'));
				}
				$pjUserModel = pjUserModel::factory();
				if (!in_array($_POST['column'], $pjUserModel->getI18n()))
				{
					$pjUserModel->set('id', $_GET['id'])->modify(array($_POST['column'] => $_POST['value']));
				} else {
	...
	public function pjActionUpdate()
	{
		...
		if ($this->isAdmin())
		{
			...
			{
				$required = array('id', 'email', 'password', 'name', 'status');
				foreach ($required as $index)
				{
				...
				if ($this->getUserId() == $_POST['id'] && isset($_POST['status']))
				{
					unset($_POST['status']);
				}
				$data = array();
