...
if (isset($tpl['status']))
{
	...
	<form action="<?php echo $_SERVER['PHP_SELF']; ?>?controller=pjAdminUsers&amp;action=pjActionUpdate" method="post" id="frmUpdateUser" class="form pj-form" autocomplete="off">
		...
		<p>
			...
			<span class="pj-form-field-custom pj-form-field-custom-before">
				...
				<input type="text" name="email" id="email" class="pj-form-field required email w200" autocomplete="off" value="<?php echo pjSanitize::html($tpl['arr']['email']); ?>" />
			</span>
		</p>
		<p>
			...
			<span class="pj-form-field-custom pj-form-field-custom-before">
				...
				<input type="password" name="password" id="password" class="pj-form-field required w200" autocomplete="off" value="password" />
			</span>
		</p>
		<p>
		...
		<p>
			<label class="title"><?php __('lblNotifications'); ?></label>
			<span class="inline_block">
				<label class="content"><input type="checkbox" name="is_notify" id="is_notify" class="pj-form-field"<?php echo $tpl['arr']['is_notify'] == 'T' ? ' checked="checked"' : null;?>/></label>
			</span>
		</p>
		<?php 
		if ($tpl['arr']['id'] != $controller->getUserId()){ 
		?>
			<p>
			...
			?><input type="hidden" name="status" value="<?php echo $tpl['arr']['status']; ?>" /><?php
		}
		?>
		<p>
